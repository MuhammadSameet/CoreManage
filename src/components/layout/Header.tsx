...
export function Header({ opened, toggle }: HeaderProps) {
    ...
    const initials = userName.split(' ').filter(Boolean).map(n => n[0]).join('').toUpperCase().substring(0, 2) || 'U';
                ...
                <Menu shadow="md" width={200} position="bottom-end">
                    <Menu.Target>
                        <UnstyledButton className="flex items-center gap-2 pl-2 py-1 rounded-full hover:bg-gray-50 transition-colors">
                            <Avatar src={null} alt={userName} radius="xl" color="blue" size={36}>{initials || 'U'}</Avatar>
                            <div className="hidden sm:block text-left">
                                <Text size="sm" fw={500} lh={1}>{userName}</Text>
